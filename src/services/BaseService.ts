...
export abstract class BaseService {
  ...
  async importData<T>(records: T[]): Promise<{
    ...
    if (this.table === TableEnum.SETTINGS) {
      await Promise.all(
        validRecords.map((record) =>
          this.db.table(TableEnum.SETTINGS).put(record),
        ),
      )
    } else {
    ...
    if (this.isParentTable()) {
      ...
      await Promise.all(
        parentIds.map((parentId) => this.updateLastChild(parentId)),
      )
    } else if (this.isChildTable()) {
      // Ensure lastChild property is updated for each parent record
   ...
   */
  async toggleFavorite<T extends { id: IdType; status: StatusEnum[] }>(
    record: T,
  ): Promise<void> {
