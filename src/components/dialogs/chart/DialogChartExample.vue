...
const chartDataThreeMonths: ComputedRef<
...
const chartDataOneYear: ComputedRef<
  ChartData<'line', { x: number; y: number }[]>
> = computed(() => {
  return {
    datasets: [
...
const chartDataAllTime: ComputedRef<
  ChartData<'line', { x: number; y: number }[]>
> = computed(() => {
  return {
    datasets: [
...
onMounted(async () => {
  ...
  hasRecordsBeyondThreeMonths.value =
    exampleResultDatasets.hasRecordsBeyondThreeMonths
