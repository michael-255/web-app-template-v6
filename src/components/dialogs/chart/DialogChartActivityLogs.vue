...
const chartData: ComputedRef<ChartData<'scatter', { x: number; y: number }[]>> =
  computed(() => {
    const infoLogs = liveRecords.value.filter(
      (record) => record.logLevel === LogLevelEnum.INFO,
    )
    const warnLogs = liveRecords.value.filter(
      (record) => record.logLevel === LogLevelEnum.WARN,
    )
    const errorLogs = liveRecords.value.filter(
      (record) => record.logLevel === LogLevelEnum.ERROR,
    )
    return {
      datasets: [
        {
