...
it('tableColumn', () => {
  ...
  expect(
    (textColumn.format as any)(
      'A very long description that exceeds forty characters',
    ),
  ...
  expect((listPrintColumn.format as any)(['item1', 'item2', 'item3'])).toBe(
    'item1, item2, item3',
  )
  ...
  expect((settingColumn.format as any)('any-string-or-enum')).toBe(
    'any-string-or-enum',
  )
})
...
it('timeAgo', () => {
  ...
  expect(timeAgo(now - 30000)).toEqual({
    message: 'just now',
    color: 'primary',
  })
